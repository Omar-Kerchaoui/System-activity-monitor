...
float LinuxParser::MemoryAvailable()
{
    float total_mem, available_mem;
    string line, key, value;
    std::ifstream filestream(kProcDirectory + kMeminfoFilename);
    ...
    return (available_mem) / total_mem;
}
...
std::vector<DiskInfo> LinuxParser::getDiskInfo()
{
    ...
    while (std::getline(file, line))
    {
        ...
        int sizeInt = stoi(blocks);

        if (!name.empty())
        {
            diskInfo.push_back(DiskInfo(blocks, name));
        }
    }
...
std::unordered_map<int, ProcessIoInfo> LinuxParser::getDiskIoInfo()
{
    std::unordered_map<int, ProcessIoInfo> ioInfo;
    std::multimap<long long, int, std::greater<long long>> topProcesses;
    ...
    for (const auto &entry : std::filesystem::directory_iterator("/proc"))
    {
        ...
        if (std::all_of(dirName.begin(), dirName.end(), isdigit))
        {
            ...
            std::ifstream file("/proc/" + dirName + "/io");
            std::string line;

            ProcessIoInfo processIO;
            ...
            while (std::getline(file, line))
            {
                ...
                if (iss >> key >> value)
                {
                    if (key == "rchar:")
                    {
                        processIO.readBytes = std::stoll(value);
                    }
                    else if (key == "wchar:")
                    {
                        processIO.writeBytes = std::stoll(value);
                    }
                }
            }
            ...
            std::ifstream nameFile("/proc/" + dirName + "/comm");
            std::string processName;
            std::getline(nameFile, processName);
            nameFile.close();

            processIO.name = processName;

            topProcesses.emplace(processIO.readBytes, pid);
            ioInfo[pid] = processIO;
    ...
    // Store the top 10 processes
    std::unordered_map<int, ProcessIoInfo> top10Processes;

    auto it = topProcesses.begin();
    int count = 0;

    while (it != topProcesses.end() && count < 20)
    {
        int pid = it->second;
        top10Processes[pid] = ioInfo[pid];
        ++it;
        ++count;
    }
